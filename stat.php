...
$eredmeny = mysqli_query($link, "SELECT floor(sum(m.prot/1000*h.gr)*4+sum(m.carb/1000*h.gr)*4+sum(m.fat/1000*h.gr)*7) as cal " . $sql_join . $profile_query . $date_query . "' AND h.blds_id =0;");
$consumed_cal_b = mysqli_fetch_array($eredmeny)['cal'];
if (!$consumed_cal_b) $consumed_cal_b = 0;
$eredmeny = mysqli_query($link, "SELECT floor(sum(m.prot/1000*h.gr)*4+sum(m.carb/1000*h.gr)*4+sum(m.fat/1000*h.gr)*7) as cal " . $sql_join . $profile_query . $date_query . "' AND h.blds_id =1;");
$consumed_cal_l = mysqli_fetch_array($eredmeny)['cal'];
if (!$consumed_cal_l) $consumed_cal_l = 0;
$eredmeny = mysqli_query($link, "SELECT floor(sum(m.prot/1000*h.gr)*4+sum(m.carb/1000*h.gr)*4+sum(m.fat/1000*h.gr)*7) as cal " . $sql_join . $profile_query . $date_query . "' AND h.blds_id =2;");
$consumed_cal_d = mysqli_fetch_array($eredmeny)['cal'];	
if (!$consumed_cal_d) $consumed_cal_d = 0;
$eredmeny = mysqli_query($link, "SELECT floor(sum(m.prot/1000*h.gr)*4+sum(m.carb/1000*h.gr)*4+sum(m.fat/1000*h.gr)*7) as cal " . $sql_join . $profile_query . $date_query . "' AND h.blds_id =3;");
$consumed_cal_s = mysqli_fetch_array($eredmeny)['cal'];
if (!$consumed_cal_s) $consumed_cal_s = 0;
?>